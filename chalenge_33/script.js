...
const informationgenerale=document.getElementById('informationgenerale');
const listname=document.createElement('ul');

const listemployee=employees.reduce((acc,elm)=>{
    
    return acc+`<li>${elm.name}-${elm.role}</li>`;
},'');
listname.innerHTML=listemployee;
informationgenerale.append(listname);