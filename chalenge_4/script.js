...
const rsulta=document.getElementById('actusemployee')
employees.forEach(employe => {
  ...
  const statu=document.createElement('span');
         ...
         if(employe.id==3){
             element.innerHTML+="name: "+employe.name;
             element.append(statu);
             statu.textContent= "|inactive"
             statu.style.color="red";
            }else{
                element.innerHTML+="name: "+employe.name;
                element.append(statu);
                statu.textContent= "|active"
